...
from models import BiViewModel
...
def main():
    ...
    model = BiViewModel(input_dim=data.feature_dim).to(device)
    optimizer = Adam(
        model.parameters(), 
        lr=config.LR, 
    ...
    logger.info(f"=== 阶段1：二视图对比预训练 ({contrast_epochs}轮) ===")
    best_loss = float('inf')
    ...
    for epoch in contrast_pbar:
        ...
        for batch in train_loader:
            ...
            contrast_loss, align_loss = model(batch, task="contrast")
            
            loss = contrast_loss + align_loss * config.ALIGN_WEIGHT
